...
public class Test調剤 {
	...
	private static final String TENSU = "調剤";
	...
	private static final String COMMON_TITLE = GENGO + YEAR + "年" + TENSU + "報酬点数表";
	private static final String OLD_URL = "../../" + OLD_YEAR + "/" + TEN;
	private static final String OLD_NAME = OLD_GENGO + OLD_YEAR + "年版";

	private static final String[] KOKUJI_PDF = {
		PDF_DIR + "0000196304.pdf",	// 別表第3 （調剤点数表）
	};

	private static final String[] TUTI_PDF = {
		PDF_DIR + "0000196311.pdf",	// 別添3（調剤点数表）
	};

	private static final String YOSHIKI_PDF = PDF_DIR + "0000196312.pdf";
		...
		renderer.HTML出力(doc, new File(HTML_DIR), dict,
			new RendererOption() {
				...
				public void 目次(Node node, TextWriter w) {
					w.printf("<hr>%n");
					w.printf("<a class='link-menu' href='%s/%s.html' target='_top'>→ %s</a>%n", OLD_URL, node.fileName(), OLD_NAME);
					w.printf("<a class='link-menu' href='hikaku.html?%s' target='_top'>→ 比較</a>%n", node.fileName());
					w.printf("<a class='link-menu' href='kubun.html#%s' target='_top'>→ 区分番号一覧</a>%n", node.fileName());
					w.printf("<a class='link-menu' href='../k/0.html' target='_top'>→ 施設基準</a>%n");
				}
			});
...
//		PdfExtractor extractor = new PdfExtractor();
//		extract(extractor, PDF_KIHON_YOSIKI, PDF_KIHON_FORMAT);
	    List<別添> list = Pdf.split(YOSHIKI_PDF, true, 4, 10, OUTPUT_PDF_DIR, new 別添関数() {
            @Override
	    ...
	    renderer.HTML出力(list, new File(HTML_DIR), new 別添RendererCallback() {
