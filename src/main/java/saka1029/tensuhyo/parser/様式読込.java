...
public class 様式読込 {
	...
	static final Pattern YOSHIKI_NAME = Pattern.compile("\\s*[(（]?\\s*(" // group1
	...
	public static List<様式> 施設基準(String outTxtFile, String... inPdfFiles) throws IOException {
			...
			for (String inPdfFile : inPdfFiles) {
				...
				for (int pageCount = pageLines.size(); i < pageCount; ++i) {
					...
					for (int j = 0, lineCount = Math.min(HEAD_LINES, lines.size()); j < lineCount; ++j) {
						...
						if (m.matches()) {
							if (name != null) {
								result.add(new 様式(name, id, startPage, i, title));
