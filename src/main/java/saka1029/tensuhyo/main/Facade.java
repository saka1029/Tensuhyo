...
import saka1029.tensuhyo.pdf.IText;
...
public class Facade {
	...
	public void 施設基準告示変換() throws IOException {
	    new IText(false).テキスト変換(テキストパス("k", "kokuji"), PDFパス("k", 施設基準告示PDF));
	}

	public void 施設基準通知変換() throws IOException {
	    new IText(true).テキスト変換(テキストパス("k", "tuti"), PDFパス("k", 施設基準通知PDF));
	}

	public void 施設基準基本様式変換() throws IOException {
	    new IText(true).様式一覧変換(テキストパス("k", "kihon-yoshiki"), PDFパス("k", 施設基準基本様式PDF));
	}

	public void 施設基準特掲様式変換() throws IOException {
	    new IText(true).様式一覧変換(テキストパス("k", "tokkei-yoshiki"), PDFパス("k", 施設基準特掲様式PDF));
	}
...
//    }
	...
	static final String 様式プレフィックス = "BESI";
	static final String 基本様式プレフィックス = "KIHON-BETTEN7-BESI";
	static final String 特掲様式プレフィックス = "TOKKEI-BETTEN2-BESI";

	public void 施設基準基本様式() throws IOException, DocumentException {
...
//            @Override public String baseUrl() { return ベースURL("k"); }
//        });
		List<様式> list = IText.ページ分割(テキストパス("k", "基本様式"), HTMLパス("k") + "/image", 基本様式プレフィックス);
		new 様式一覧Renderer().HTML出力(list, new File(HTMLパス("k")),
			元号 + 年度 + "年 基本診療料の施設基準に係る届出書", "1.betten", ベースURL("k"), 基本様式プレフィックス);
	}
...
//	}
	...
	public void 施設基準特掲様式() throws IOException, DocumentException {
//	    Map<String, 別添> map = new LinkedHashMap<>();
...
//            @Override public String baseUrl() { return ベースURL("k"); }
//        });
		List<様式> list = IText.ページ分割(テキストパス("k", "特掲様式"), HTMLパス("k") + "/image", 特掲様式プレフィックス);
		new 様式一覧Renderer().HTML出力(list, new File(HTMLパス("k")),
			元号 + 年度 + "年 特掲診療料の施設基準に係る届出書", "2.betten", ベースURL("k"), 特掲様式プレフィックス);
	}
...
//            }
//	    });
	    new IText(true).テキスト変換(テキストパス("i", "kokuji"), PDFパス("i", 医科告示PDF));
    }
...
//            }
//	    });
	    new IText(true).テキスト変換(テキストパス("i", "tuti"), PDFパス("i", 医科通知PDF));
    }
    
    public void 医科様式変換() throws IOException {
	    new IText(true).様式一覧変換(テキストパス("i", "yoshiki"), PDFパス("i", 医科様式PDF));
    }
...
//	};
	...
	public void 医科様式() throws IOException, DocumentException {
//	    Map<String, 別添> map = new LinkedHashMap<>();
...
//            @Override public String baseUrl() { return ベースURL("i"); }
//        });
		List<様式> list = IText.ページ分割(テキストパス("i", "様式"), HTMLパス("i") + "/image", 様式プレフィックス);
		new 様式一覧Renderer().HTML出力(list, new File(HTMLパス("i")),
			元号 + 年度 + "年 別紙様式一覧", "yoshiki", ベースURL("i"), 様式プレフィックス);
	}
...
//	    });
	    new IText(true).テキスト変換(テキストパス("s", "kokuji"), PDFパス("s", 歯科告示PDF));
	}
...
//            @Override public String eval(String line) { return line.matches("^\\s*\\S*\\s*-\\s*[0-9０-９]+\\s*-$") ? "#" + line : line; }
//	    });
	    new IText(true).テキスト変換(テキストパス("s", "tuti"), PDFパス("s", 歯科通知PDF));
	}

	public void 歯科様式変換() throws IOException {
	    new IText(true).様式一覧変換(テキストパス("s", "yoshiki"), PDFパス("s", 歯科様式PDF));
	}
...
//	}
	...
	public void 歯科様式() throws IOException, DocumentException {
//	    Map<String, 別添> map = new LinkedHashMap<>();
...
//            @Override public String baseUrl() { return ベースURL("s"); }
//        });
		List<様式> list = IText.ページ分割(テキストパス("s", "様式"), HTMLパス("s") + "/image", 様式プレフィックス);
		new 様式一覧Renderer().HTML出力(list, new File(HTMLパス("s")),
			元号 + 年度 + "年 別紙様式一覧", "yoshiki", ベースURL("s"), 様式プレフィックス);
	}
...
//            }
//	    });
	    new IText(true).テキスト変換(テキストパス("t", "kokuji"), PDFパス("t", 調剤告示PDF));
	}
...
//            }
//	    });
	    new IText(true).テキスト変換(テキストパス("t", "tuti"), PDFパス("t", 調剤通知PDF));
	}

	public void 調剤様式変換() throws IOException {
	    new IText(true).様式一覧変換(テキストパス("t", "yoshiki"), PDFパス("t", 調剤様式PDF));
	}
...
//    };
	...
	public void 調剤様式() throws IOException, DocumentException {
//	    Map<String, 別添> map = new LinkedHashMap<>();
...
//            @Override public String baseUrl() { return ベースURL("t"); }
//        });
		List<様式> list = IText.ページ分割(テキストパス("t", "様式"), HTMLパス("t") + "/image", 様式プレフィックス);
		new 様式一覧Renderer().HTML出力(list, new File(HTMLパス("t")),
			元号 + 年度 + "年 別紙様式一覧", "yoshiki", ベースURL("t"), 様式プレフィックス);
	}
}
